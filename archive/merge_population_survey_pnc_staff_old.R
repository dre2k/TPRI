...
source(glue("{data_directory}/{date}/USCVaccineStudyStaff_R.r"))
...
names(staffData) <- paste0("PSS_", toupper(names(staffData)))
...
# add USCID to W1W2 nonmatches (only found in W2)
# you had to do this because there are some individuals who match on W2 but not W1
# this script is meant to add USCID to the W1/POPsurvey participants so they're included at all levels
source("E:/USCVaccineStudy/merge_population_survey_staff_w1_clean.R")
...
output <- match_survey_pop(survey_data = completeStaffData, 
                           pnc_data = populationData,
                           survey_uscid = PSS_USC_ID, 
                           pnc_uscid = POP_USCID,
...
check <- output %>% 
  ...
  dplyr::select(USCID, PSS_FIRST_NAME, POP_PATIENTFIRST, PSS_LAST_NAME, POP_PATIENTLAST, 
                POP_DOB, PSS_EMAIL1, POP_EMAIL1, PSS_RECORD_ID) %>% 
  # email typo that causes individual to match another person
  dplyr::filter(PSS_LAST_NAME != POP_PATIENTLAST)
...
saveRDS(output, file = glue("E:/USCVaccineStudy/output/{date}/population_survey_staff_{date}.rds"))
...
saveRDS(output_deidentified, file = glue("E:/USCVaccineStudy/output/{date}/population_survey_staff_deidentified_{date}.rds"))
write.csv(output_deidentified, file = glue("E:/USCVaccineStudy/output/{date}/population_survey_staff_deidentified_{date}.csv"))
